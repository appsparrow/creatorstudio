...
// src/index.ts
function personaToDb(persona, userId) {
  return {
    id: persona.id,
    user_id: userId,
...
function dbToPersona(row) {
  const psychographic = row.psychographic ?? {
    coreTraits: [],
    interests: [],
    values: [],
    fears: [],
  ...
  const fashionStyle = row.fashion_style ?? {
    aesthetic: "",
    signatureItems: [],
    photographyStyle: ""
  };
...
function dayToDb(day, userId) {
  return {
    ...
    source: day.source ?? "studio",
    ugc_run_id: day.ugcRunId ?? null,
    product_url: day.productUrl ?? null
  };
}
__name(dayToDb, "dayToDb");
function dbToDay(row) {
  return {
    id: row.id,
    ...
    contentType: row.content_type ?? "Photo",
    theme: row.theme ?? "",
    sceneDescription: row.scene_description ?? "",
    onScreenText: row.on_screen_text ?? "",
    hairstyle: row.hairstyle ?? void 0,
    styleOption: row.style_option ?? void 0,
    caption: row.caption ?? "",
    hook: row.hook ?? "",
    hashtags: row.hashtags ?? "",
    cta: row.cta ?? "",
    location: row.location ?? "",
    musicSuggestion: row.music_suggestion ?? "",
    notes: row.notes ?? "",
    generatedImageUrl: row.generated_image_url ?? void 0,
    generatedVideoUrl: row.generated_video_url ?? void 0,
    customMediaUrl: row.custom_media_url ?? void 0,
    pendingVideoTaskId: row.pending_video_task_id ?? void 0,
    status: row.status ?? "draft",
    isAIGenerated: row.is_ai_generated ?? void 0,
    isGoodToPost: row.is_good_to_post ?? void 0,
    postImageReferences: row.post_image_references ?? void 0,
    slides: row.slides ?? void 0,
    source: row.source ?? "studio",
    ugcRunId: row.ugc_run_id ?? void 0,
    productUrl: row.product_url ?? void 0
  };
}
__name(dbToDay, "dbToDay");
function userClient(env2, token) {
  return createClient(env2.SUPABASE_URL, env2.SUPABASE_ANON_KEY, {
    global: { headers: { Authorization: `Bearer ${token}` } },
    auth: { persistSession: false, autoRefreshToken: false }
  });
}
__name(userClient, "userClient");
function serviceClient(env2) {
  return createClient(env2.SUPABASE_URL, env2.SUPABASE_SERVICE_ROLE_KEY, {
    auth: { persistSession: false, autoRefreshToken: false }
  });
}
...
async function resolveUserId(supabase) {
  ...
  if (error3 || !user) throw new Error("Unauthorized \u2014 invalid token");
  return user.id;
}
__name(resolveUserId, "resolveUserId");
async function generateKlingJWT(apiKey, apiSecret) {
  const b64url = /* @__PURE__ */ __name((input) => btoa(input).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_"), "b64url");
  const header = b64url(JSON.stringify({ alg: "HS256", typ: "JWT" }));
  const payload = b64url(JSON.stringify({
    iss: apiKey,
    exp: Math.floor(Date.now() / 1e3) + 1800,
    nbf: Math.floor(Date.now() / 1e3) - 60
  }));
...
__name(generateKlingJWT, "generateKlingJWT");
function extractFolderId(url) {
  const match2 = url.match(/\/folders\/([-_a-zA-Z0-9]+)/);
  return match2 ? match2[1] : null;
}
__name(extractFolderId, "extractFolderId");
var appWithVars = new Hono2();
...
appWithVars.get("/api/personas", async (c) => {
  ...
  const { data, error: error3 } = await supabase.from("personas").select("*").order("created_at", { ascending: true });
  if (error3) return c.json({ error: error3.message }, 500);
  const personas = (data ?? []).map((row) => dbToPersona(row));
...
appWithVars.post("/api/personas", async (c) => {
  ...
  const { error: error3 } = await supabase.from("personas").upsert(row, { onConflict: "id" });
  if (error3) return c.json({ error: error3.message }, 500);
...
appWithVars.delete("/api/personas/:id", async (c) => {
  ...
  const { error: daysErr } = await supabase.from("days").delete().eq("persona_id", id);
  if (daysErr) return c.json({ error: daysErr.message }, 500);
  const { error: error3 } = await supabase.from("personas").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
appWithVars.get("/api/days", async (c) => {
  const supabase = c.get("supabase");
  const { data, error: error3 } = await supabase.from("days").select("*").order("created_at", { ascending: true });
  if (error3) return c.json({ error: error3.message }, 500);
  const days = (data ?? []).map((row) => dbToDay(row));
  return c.json(days);
});
appWithVars.post("/api/days", async (c) => {
  ...
  const { error: error3 } = await supabase.from("days").upsert(row, { onConflict: "id" });
  if (error3) return c.json({ error: error3.message }, 500);
...
appWithVars.delete("/api/days/:id", async (c) => {
  const supabase = c.get("supabase");
  const id = c.req.param("id");
  const { error: error3 } = await supabase.from("days").delete().eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
appWithVars.get("/api/settings", async (c) => {
  const supabase = c.get("supabase");
  const userId = c.get("userId");
  const { data, error: error3 } = await supabase.from("user_settings").select("*").eq("user_id", userId).single();
...
appWithVars.post("/api/settings", async (c) => {
  ...
  const { error: error3 } = await supabase.from("user_settings").upsert(row, { onConflict: "user_id" });
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
appWithVars.post("/api/images/save", async (c) => {
  ...
  const storagePath = personaId ? `${userId}/${personaId}/${filename}` : `${userId}/${filename}`;
  try {
  ...
  const publicUrl = c.env.R2_PUBLIC_URL ? `${c.env.R2_PUBLIC_URL}/${storagePath}` : `/api/media/${storagePath}`;
...
appWithVars.post("/api/videos/save", async (c) => {
  ...
  const storagePath = personaId ? `${userId}/${personaId}/${safeFilename}` : `${userId}/${safeFilename}`;
...
appWithVars.post("/api/nanobanana/proxy", async (c) => {
  ...
  const response = await fetch(`https://api.nanobananaapi.ai${endpoint}`, {
    ...
    body: httpMethod === "GET" ? void 0 : JSON.stringify(payload)
  });
...
appWithVars.post("/api/videos/generate", async (c) => {
  ...
  const callbackUrl = publicTunnelUrl ? `${publicTunnelUrl.replace(/\/$/, "")}/api/videos/callback` : void 0;
  ...
  const taskId = data.task_id ?? taskData?.task_id;
  if (!taskId) {
    throw new Error(
  ...
  if (dayId) {
    const { error: taskErr } = await supabase.from("video_tasks").upsert({ task_id: taskId, day_id: dayId, user_id: userId }, { onConflict: "task_id" });
    if (taskErr) {
...
appWithVars.get("/api/videos/status/:taskId", async (c) => {
  ...
  const taskResult = statusData.data?.task_result;
  const videos = taskResult?.videos;
  const taskStatus = statusData.data?.task_status;
  const videoUrl = videos?.[0]?.url ?? taskResult?.video_url ?? statusData.data?.video_url;
  return c.json({
    raw: statusData,
...
appWithVars.post("/api/videos/callback", async (c) => {
  ...
  const { data: taskRow, error: taskErr } = await supabase.from("video_tasks").select("day_id, user_id").eq("task_id", taskId).single();
  if (taskErr || !taskRow) {
    console.log(`[Webhook] No day mapping found for task_id=${taskId}`);
    return c.json({ success: true });
  }
  const { day_id, user_id } = taskRow;
  const { error: updateErr } = await supabase.from("days").update({
...
appWithVars.post("/api/blotato/publish", async (c) => {
  ...
  const targetAccount = accountsArray.find((acc) => acc.platform === "instagram") ?? accountsArray[0];
  if (!targetAccount?.id) {
    return c.json(
  ...
  const processMedia = /* @__PURE__ */ __name(async (mediaPath) => {
    let bytes;
    ...
    if (mediaPath.startsWith("http")) {
      ...
      const resolvedPath = pathParts.length >= 2 ? `${userId}/${pathParts.join("/")}` : `${userId}/${storagePath}`;
  ...
  if (hashtags) {
    const tagsArray = hashtags.split(/[\s#]+/).filter(Boolean).map((t) => `#${t}`);
...
appWithVars.post("/api/drive/list", async (c) => {
  ...
  do {
    ...
    const page = await driveRes.json();
  ...
  if (upsertRows.length > 0) {
    const { error: upsertErr } = await supabase.from("drive_assets").upsert(upsertRows, {
...
appWithVars.get("/api/drive/assets", async (c) => {
  ...
  let query = supabase.from("drive_assets").select("*").order("synced_at", { ascending: false });
...
appWithVars.patch("/api/drive/assets/:id", async (c) => {
  ...
  const { error: error3 } = await supabase.from("drive_assets").update(updates).eq("id", id);
  if (error3) return c.json({ error: error3.message }, 500);
  return c.json({ success: true });
});
...
appWithVars.post("/api/claude/messages", async (c) => {
  const supabase = c.get("supabase");
  const userId = c.get("userId");
  const { data: settingsRow } = await supabase.from("user_settings").select("anthropic_api_key").eq("user_id", userId).single();
...
appWithVars.post("/api/ugc/generate", async (c) => {
  ...
  const callClaude = /* @__PURE__ */ __name(async (model, system, prompt) => {
  ...
  const libraryHooks = (libItems || []).filter((i) => i.item_type === "viral_hook");
  const libraryFormats = (libItems || []).filter((i) => i.item_type === "content_format");
  const libraryRules = (libItems || []).filter((i) => i.item_type === "decision_rule");
  const librarySettings = (libItems || []).filter((i) => i.item_type === "location_setting");
  const hooksContext = libraryHooks.map((h) => `${h.label} (score: ${h.performance_score}): "${h.data.structure}" \u2014 Best for: ${(h.data.bestFor || []).join(", ")}. Trigger: ${h.data.triggerCondition || ""}`).join("\n") || "price_reveal, pov, discovery, social_proof, comparison, opinion";
  const rulesContext = libraryRules.map((r) => `${r.data.category}: hook=${(r.data.recommendedHook || "").replace("hook_", "")} format=${(r.data.recommendedFormat || "").replace("format_", "")} \u2014 ${r.data.reasoning || ""}`).join("\n") || "";
  try {
    const productIntel = await callClaude(
      HAIKU,
...
Return JSON with: productName, brand, category, subcategory, price, currency, size, keyFeatures[], primaryBenefit, painPointsSolved[], reviewSentiment:{positive[],negative[]}, competitorProducts:[{name,price}], targetAudience, trendingStatus, sourceUrl`
    ...
    const matchingRule = libraryRules.find((r) => r.data.category === productIntel.category);
    const validSettings = matchingRule?.data?.validSettings || [];
...
Return JSON with: hookFormat, hookRationale, contentFormat, contentRationale, videoLength, setting, characterOutfit, optimalPostingTime, postingRationale, hashtagStrategy:{primary,conversion,product,brand,modifier}, decisionLog:{ruleUsed,libraryHookScore,overridden,overrideReason}`
    ...
    const chosenHook = libraryHooks.find((h) => h.slug === `hook_${strategy.hookFormat}`);
...
Return JSON with: hookVariants[{hook,score,rationale}], selectedHook, fullScript{hookSection,productSection,trustSection,ctaSection} (each with timing,wordCount,textOverlay,voiceover,visualCue), totalWordCount, estimatedDuration, elevenlabsFullScript, elevenlabsSettings`
    ...
    const settingData = librarySettings.find((s) => s.slug === randomSetting);
...
// .wrangler/tmp/bundle-pHFEM9/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-pHFEM9/middleware-loader.entry.ts
