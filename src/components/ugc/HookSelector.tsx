      ...
      {sorted.map((h, i) => (
        ...
        >
          <div className={cn(
            ...
            selectedHook === h.hook ? 'border-violet-400 bg-violet-500/20' : 'border-gray-600'
          )}>
            {selectedHook === h.hook && <div className="w-2 h-2 rounded-full bg-violet-400" />}
          </div>
          <div className="flex-1 min-w-0">
            <p className={cn(
              'text-sm',
              selectedHook === h.hook ? 'text-white' : 'text-gray-300'
            )}>
              "{h.hook}"
            </p>
            <p className="text-[11px] text-gray-500 mt-0.5">{h.rationale}</p>
          </div>
          <span className={cn(
